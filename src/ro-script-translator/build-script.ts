export class BuildScript {
  private _scripts = {};
  private _extractedScript!: { expressions: string[]; comboes: Record<string, string[]> };
  private patterns = {
    skill: ['เมื่อใช้สกิล'],
    step: ['ทุกการอัพเกรด', 'ทุก ๆ การอัปเกรด', 'ทุก ๆ การอัพเกรด', 'ทุก ๆ Base'],
    constant: ['เมื่ออัพเกรดถึงขั้น', 'เมื่ออัพเกรดขั้น', 'เมื่ออัปเกรดถึงขั้น', 'ATK', 'MSP'],
  ...
  private regex = {
    toSteps: [/ทุก.*การอัพเกรด (.+) ขั้น (.+)/, /ทุก.*การอัปเกรด (.+) ขั้น (.+)/],
    toConstants: [
  ...
  private map = {
    ...
    'เพิ่ม Damage เมื่อโจมตีมอนสเตอร์ทุกประเภท (ทั่วไป, Boss, Guardian)': 'p_class_all',
    'เพิ่ม Damage ทางกายภาพต่อศัตรูประเภทบอส': 'p_class_boss',
    'เพิ่ม Damage ทางเวทมนตร์ต่อศัตรูทุกประเภท': 'p_class_all',
    ...
    'เพิ่มพลังโจมตีทางเวทย์ที่สร้างต่อศัตรูธาตุ Neutral ขึ้น': 'm_element_neutral',
  ...
  constructor(private rawItemDescription: string) {
    ...
    this.toScripts(this._extractedScript.expressions, this._extractedScript.comboes);
  }

  getComboScript(rawExpression: string): { actualAttr?: string; bonus?: string }[] {
    const scripts = [] as { actualAttr?: string; bonus?: string }[];

    const autoRegex = /กับ.\s*(.+?)\sเมื่อโจมตี.*(มีโอกาส.*?\s)(.*)/;
    const [_raw, comboItem, isAuto, chanceScript] = rawExpression.match(autoRegex) ?? [];
    if (isAuto) {
      ...
      for (const expression of rawScript
        ...
        for (const a of this.getMiniScript(expression)) {
          scripts.push({
            ...
            bonus: `EQUIP[${comboItem}]:AUTO:${a.bonus}` + (time ? `(${time})` : ''),
          });
        }
      }
    ...
    for (const expression of expressions.filter((a) => a.match(/\d/))) {
      ...
      for (const a of this.getMiniScript(expression)) {
        scripts.push({ actualAttr: a.actualAttr, bonus: `EQUIP[${combo}]===${a.bonus}` });
      }
    }
  ...
  getAutoByAtkScript(rawExpression: string): { actualAttr?: string; bonus?: string }[] {
    const scripts = [] as { actualAttr?: string; bonus?: string }[];
        ...
        if (Array.isArray(buffs) && buffs.length > 0) {
          for (const { actualAttr, bonus } of buffs) {
            scripts.push({
              ...
              bonus: `ACTION[${action}]:AUTO:${bonus}` + (time ? `(${time})` : ''),
            });
          }
        } else {
          scripts.push({
            actualAttr: 'AUTO_SKILL',
            bonus: `ACTION[${action}]:AUTO:${expression}`,
          });
        }
      }
    }
  ...
  getAutoBySkillScript(rawExpression: string): { actualAttr?: string; bonus?: string }[] {
    const scripts = [] as { actualAttr?: string; bonus?: string }[];
      ...
      for (const expression of rawScript.split(',').map((a) => a.trim())) {
        ...
        if (Array.isArray(buffs) && buffs.length > 0) {
          for (const { actualAttr, bonus } of buffs) {
            scripts.push({
              actualAttr: actualAttr,
              bonus: `SKILL[${skillName}]==${bonus}` + (time ? `(${time})` : ''),
            });
          }
        } else {
          scripts.push({
  ...
  matchRefineConstantBonus(rawExpression: string): { condition: string; script: string } | undefined {
  ...
  matchRefineStepBonus(rawExpression: string): { every: string; bonusScript: string } | undefined {
    const usable = rawExpression.trim();
  ...
  getMiniScript(rawExpression: string): { actualAttr?: string; bonus?: string }[] {
    ...
    if (attr && bonus) {
      ...
      const goodBonus = Number.isNaN(Number(bonus)) ? bonus.replace(/\D/g, '') : bonus;
      // console.log({actualAttr, bonus, goodBonus})
      if (
        ['atk', 'matk', 'mhp', 'maxhp', 'hp', 'msp', 'maxsp', 'sp', 'aspd'].includes(actualAttr.toLowerCase()) &&
        bonus.includes('%')
      ) {
  ...
  toRefineConstantBonus(params?: string | { condition: string; script: string }): any[] {
    let rawExpression = '';
    let condition = '';
    if (typeof params === 'string') {
      const mached = this.matchRefineConstantBonus(params) ?? this.matchBaseBonusStat(params)!;
  ...
  toRefineStepBonus(params: { every: string; bonusScript: string }): Record<string, string>[] {
    if (!params) return [];
  ...
  toTotalRefineBonus(params: { condition: string; bonusScript: string }): Record<string, string>[] {
  ...
  isBuffWhenSkill(script: string): { skillName: string; chanceScript: string } | undefined {
    let [_raw, _text, skillName, chanceScript] = script.match(/เมื่อใช้(สกิล|งาน)\s*(.+?),\s*(.*)/) ?? [];
    if (skillName && chanceScript) {
      return { chanceScript, skillName };
    }

    [_raw, _text, skillName, chanceScript] = script.match(/เมื่อใช้(สกิล|งาน)\s*(.+)(เพิ่ม.+)/) ?? [];
    if (skillName && chanceScript) {
      return { chanceScript, skillName };
    }
  ...
  isLevelStep(expression: string): { condition: string; script: string } | undefined {
  ...
  matchStepBonusStat(expression: string): { every: string; bonusScript: string } | undefined {
    const regexs = [
  ...
  matchBaseBonusStat(expression: string): { condition: string; script: string } | undefined {
    const regexs = [
  ...
  matchTotalRefine(expression: string): { condition: string; bonusScript: string } | undefined {
  ...
  extractItemExpressionTh2(itemDescription: string): this {
    const splited = itemDescription
      ...
      .filter((a) => this.matchBonusScript(a) || this.matchRefineConstantBonus(a) || this.matchRefineStepBonus(a));
    // console.log({splitedDoubleSlash: splited})

    const patternNoCombo = this.statePatterns.filter((a) => !this.patterns.itemSet.includes(a));
    const regexConditionNoCombo = new RegExp(patternNoCombo.join('s*.+|') + ' .+');
    ...
    for (const ss of splited) {
      ...
      for (const ss2 of splited2) {
        ...
        if (curComboCondition) {
          if (!ss2.match(/เมื่อค่าอัพเกรดรวมกัน/) && !ss2.match(/ค่าอัพเกรดของ.+รวมกัน.+/)) {
        ...
        } else if (/เมื่อโจมตี.+มีโอกาส/.test(ss2) && regexConditionNoCombo.test(ss2)) {
          expressions.push(ss2);
  ...
  toScripts(expressions: string[], comboes: Record<string, string[]>) {
    ...
    for (const expression of expressions) {
      ...
      if (this.isCombo(expression)) {
        ...
        // console.log({expression})
        for (const obj of this.toRefineStepBonus(this.matchRefineStepBonus(expression)!)) {
          const [prop, newScript] = Object.entries(obj)[0];
          addScript(prop, newScript);
        }
      } else if (this.matchRefineConstantBonus(expression)) {
        // console.log({expression})
        for (const obj of this.toRefineConstantBonus(this.matchRefineConstantBonus(expression)!)) {
          const [prop, newScript] = Object.entries(obj)[0];
        ...
        // console.log({expression})
        for (const obj of this.toRefineStepBonus(this.matchStepBonusStat(expression)!)) {
          const [prop, newScript] = Object.entries(obj)[0];
          addScript(prop, newScript as any);
        }
      } else if (this.matchBaseBonusStat(expression)) {
        // console.log({expression})
        for (const obj of this.toRefineConstantBonus(this.matchBaseBonusStat(expression)!)) {
          const [prop, newScript] = Object.entries(obj)[0];
          addScript(prop, newScript as any);
        }
      } else {
    ...
    for (const [comboCondition, _expressions] of Object.entries(comboes)) {
      ...
      for (const expression of _expressions) {
        if (expression.match(/ทุก.*การอัพเกรด/)) {
          ...
          for (const obj of this.toTotalRefineBonus(this.matchTotalRefine(expression)!)) {
            const [prop, newScript] = Object.entries(obj)[0];
            addScript(prop, `EQUIP[${xComboCondition}]${newScript}`);
          }
        } else {
          for (const s of expression.split(',')) {
            for (const { actualAttr, bonus } of this.getMiniScript(s.trim())) {
              addScript(actualAttr as string, `EQUIP[${xComboCondition}]${bonus}`);
            }
          }
        }
      }
        ...
        const newValues = values
          ...
          .map((a) => {
            const [_, _raw, status, statusCond] =
              a.match(/(.*?)(str|dex|vit|luk|int|agi|lv|level)(\d{1,3})(===|---)(.+)/) ?? [];

            // console.log({status, statusCond, sperator, bonus})
            if (!status || !statusCond) return a;

            return a.replace(`${status}${statusCond}`, `${status}:${statusCond}`);
          });
        if (script[goodAttr]) {
  ...
  toAttr(rawAttr: string): string[] {
    const goodAttr = rawAttr
      ...
      .replace('ลดคูลดาวน์สกิล', 'cd__')
      .replace('ลดการใช้ SP ของสกิล', 'spCost__')
      .replace('ลดการใช้ SP ของสกิล ', 'spCost__')
      .replace('ลดปริมาณการใช้ SP ในการใช้สกิล ', 'spCost__')
      .replace('ลดปริมาณการใช้ SP สกิล ', 'spCost__')
    ...
    const mapPrefix = {
      ...
      'เพิ่ม Damage ทางกายภาพต่อมอนสเตอร์ประเภท': 'p_race',
    ...
    for (const [check, pre] of Object.entries(mapPrefix)) {
      if (goodAttr.startsWith(check)) {
        ...
        if (pre === 'p_size') {
          pureAttrs = pureAttrs.map((a) => a.replace('ขนาด', '')).map((a) => this.mapSize[a]);
        }
    ...
    const mapAll = {
      เพิ่มพลังโจมตีทางกายภาพและเวทย์ต่อศัตรูธาตุ: ['p_element_{a}', 'm_element_{a}'],
      'เพิ่ม Damage ทางกายภาพ/เวทมนตร์ต่อศัตรูทุกธาตุ': ['p_element_all', 'm_element_all'],
      'เพิ่ม Damage ทางกายภาพและเวทมนตร์ต่อศัตรูทุกธาตุ': ['p_element_all', 'm_element_all'],
      'เพิ่ม Damage ทางกายภาพระยะใกล้และระยะไกล': ['range', 'melee'],
      'เพิ่ม Damage ทางกายภาพและเวทมนตร์ต่อศัตรูทุกประเภท': ['p_class_all', 'm_class_all'],
      'เพิ่ม Damage ทางกายภาพ/เวทมนตร์ต่อศัตรูทุกประเภท': ['p_class_all', 'm_class_all'],
      'เพิ่ม Damage ทางกายภาพ/เวทมนตร์ต่อศัตรูทุกขนาด': ['p_size_all', 'm_size_all'],
    };
    for (const [attrAll, template] of Object.entries(mapAll)) {
      if (goodAttr.startsWith(attrAll)) {
        ...
        return doubleAttrs.flatMap((aa) => template.map((a) => a.replace('{a}', aa)));
