...
const mapRace = ['Formless', 'Undead', 'Brute', 'Plant', 'Insect', 'Fish', 'Demon', 'DemiHuman', 'Angel', 'Dragon'];
const mapScale = ['Small', 'Medium', 'Large'];
const mapElement = ['Neutral', 'Water', 'Earth', 'Fire', 'Wind', 'Poison', 'Holy', 'Dark', 'Ghost', 'Undead'];
...
export class RoScriptTranslatorService {
  ...
  private readonly baseItemFilePath = process.env.RO_BASE_FILE_PATH
  private readonly usingItemFilePath = process.env.RO_USING_FILE_PATH
  private readonly baseMonsterFilePath = process.env.RO_BASE_FILE_PATH
  private readonly usingMonsterFilePath = process.env.RO_USING_FILE_PATH
  ...
  async loadMonster(monsterIds: number[]) {
    ...
    for (const id of monsterIds) {
      ...
      try {
        ...
        const [doublePropLevel, position] = stats.element.toString().split('').map(Number);
        ...
        const sp = Array.isArray(spawn) ? [...new Set(spawn.map((a) => a.mapname))].join(',') : String(spawn);
  ...
  syncAll() {
    ...
    for (const item of Object.values(currentData)) {
      ...
      if (!item.script) {
        ...
        currentData[item.id].script = new BuildScript(currentData[item.id].description).scripts;
      }
    }
    const s = JSON.stringify(currentData, undefined, 2);
  ...
  sync<T extends { id: number } & ItemModel>(updatedItems: T[]) {
    ...
    for (const updatedItem of updatedItems) {
      ...
      if (!currentData[updatedItem.id]?.script && !updatedItem.script) {
        currentData[updatedItem.id].script = new BuildScript(currentData[updatedItem.id].description).scripts;
      }
    }
    const s = JSON.stringify(currentData, undefined, 2);
    fs.writeFileSync(path, s);
  ...
  async loadItems(rawItemIds: number[]) {
    const itemIds: number[] = [...new Set((rawItemIds as any[]).map(Number).filter((x) => !Number.isNaN(x)))];
    const updatedItems: any[] = [];
    for (const itemId of itemIds) {
      const url = `${this.baseDbAPI}/Item/${itemId}?apiKey=${this.baseDbAPIKey}&server=thROG`;
      try {
        const { data } = await this.http.get<ItemAPIModel>(url, { timeout: 1000 * 5 }).toPromise();
        const {
          id,
          aegisName,
          name,
        ...
        console.error({ itemId, msg: error?.response?.statusText || error?.response || error?.message });
      }
    }
  ...
  private addExtra(item: ItemAPIModel & { x?: Partial<ItemAPIModel>; enchants: any[]; usableClass: string[] }) {
    const { itemTypeId, aegisName, name, description, compositionPos, requiredLevel } = item;
    ...
    if (itemTypeId === 1 && item.itemLevel == null) {
      const itemLv = Number(description.split('เลเวลของอาวุธ : ^777777')[1]?.charAt(0));
      if (Number.isInteger(itemLv)) {
        item.itemLevel = itemLv;
      }
    }

    if ((aegisName?.endsWith('_Card') || name.endsWith(' Card')) && compositionPos == null) {
      item.itemTypeId = 6;
    ...
    if (requiredLevel === null && description.includes('เลเวลที่ต้องการ : ^777777')) {
      item.requiredLevel = Number(this.getTextBetween(description, 'เลเวลที่ต้องการ : ^777777'));
    }

    if (
      !item.usableClass &&
      (itemTypeId === ItemTypeId.WEAPON ||
        itemTypeId === ItemTypeId.AMMO ||
        itemTypeId === ItemTypeId.ARMOR ||
        itemTypeId === ItemTypeId.SHADOW)
    ) {
      const wording =
        this.getTextBetween(description, 'อาชีพที่ใส่ได้ : ^777777') ||
        this.getTextBetween(description, 'อาชีพ : ^777777');

      item.usableClass = [UsedWordingMap[wording] || wording || UsedWordingMap.ทุกอาชีพ];
    }
